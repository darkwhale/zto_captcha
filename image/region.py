...
def is_mergeable(region_first, region_second):
    return True if region_first.get_value() == region_second.get_value() else False


def merge_region(region_first, region_second):
    for point in region_second.get_set():
        region_first.add(point)
